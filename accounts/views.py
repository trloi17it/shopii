...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'Chúc mừng, tài khoản đã được kích hoạt!')
        return redirect('login')
    else:
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(request, '1 liên kết đặt lại mật khẩu đã được gửi đến email của bạn!')
            return redirect('login')
        else:
            messages.error(request, 'Tài khoản này không tồn tại!')
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                messages.error(request, 'Mật khẩu hiện tại chưa hợp lệ!')
                return redirect('change_password')
        else:
